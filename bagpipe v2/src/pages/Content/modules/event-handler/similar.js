export function getSimilarElement(selected_element) {
  ...
  while (final_classlist[0].length == 0) {
    ...
    if (
      first_outer.parentElement == undefined ||
  ...
  attr_data_list = Array.from(selected_element[0].attributes).filter(
    (attribute) => {
  ...
  for (let i = 0, n = attr_data_list.length; i < n; i++) {
    let first_value = selected_element[0].getAttribute(
      attr_data_list[i].nodeName
    );
    let second_value = selected_element[1].getAttribute(
      attr_data_list[i].nodeName
    );
    if (first_value == second_value) {
      final_attr =
        '[' + attr_data_list[i].nodeName + '=' + "'" + first_value + "'" + ']';

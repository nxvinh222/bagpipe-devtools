    ...
    const columns = [
        {
            ...
            render: (text, record) => {
                ...
                if (record[props.typeColumn] != 'object' && record[props.typeColumn] != 'link')
                    return <div>{text}</div>;

                let urlParams = new URLSearchParams(window.location.search);
                urlParams.set(props.fatherIdQuery, record[props.idColumn]);
                let path =
                    showRecipeBasicPath + `${props.recipeId}` + '?' + urlParams.toString();
                return (
                    <Link
                        to={{ pathname: path }}
                        onClick={() => {
        ...
        // },
        {
            ...
            render: (text, record) => {
                let path =
                    editAttrPathWithQuery + `&${props.elementIdQuery}=${record[props.idColumn]}`;
                return (
                    <div>
                        <Button>
                            <Link
